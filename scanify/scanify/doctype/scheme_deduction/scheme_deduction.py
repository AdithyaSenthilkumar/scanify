...
class SchemeDeduction(Document):
	...
	def calculate_totals(self):
		...
		for item in self.items:
			...
			self.total_deducted_value += flt(item.deducted_value)
	...
	def apply_deduction(self):
		...
		# Set flag to allow updates after submit
		statement.flags.ignore_validate_update_after_submit = True
		...
		for deduction_item in self.items:
			...
			for stmt_item in statement.items:
				if stmt_item.product_code == deduction_item.product_code:
					# Add to free_qty (this reduces closing stock)
					stmt_item.free_qty = flt(stmt_item.free_qty) + flt(deduction_item.deduct_qty)
		...
		frappe.msgprint(
			...
			f"Updated {len(products_updated)} products: {', '.join(products_updated)}<br>"
	...
	def reverse_deduction(self):
		...
		for deduction_item in self.items:
			for stmt_item in statement.items:
				if stmt_item.product_code == deduction_item.product_code:
					# Subtract from free_qty (reverses the deduction)
					stmt_item.free_qty = flt(stmt_item.free_qty) - flt(deduction_item.deduct_qty)
					break
		...
		frappe.msgprint(
			f"⚠️ Deduction reversed from {self.stockist_statement}",
			alert=True,
			indicator="orange"
		)
